...
class HDF5VLADataset:
    ...
    def __init__(self) -> None:
        ...
        HDF5_DIR = "/Data/lzl/simulated_np_data/libero/libero_spatial_no_noops"
        ...
        for traj_dir in tqdm(hdf5_list, desc="Loading datalist from dataset"):
            traj_dir_path = os.path.join(HDF5_DIR, traj_dir)
            # task_list = os.listdir(task_dir_path)
            # for file in task_list:
            self.file_paths.append(traj_dir_path)
    ...
    def parse_hdf5_file(self, file_path):
        ...
        frame_path = os.path.join(file_path, 'frames_primary.npy')
        action_path = os.path.join(file_path, 'actions.npy')
        qpos_path = os.path.join(file_path, "states.npy")
        ...
        instruction_path = os.path.join(file_path, "task.txt")
        ...
        state = self.get_ortho6d_from_euler_angle(qpos)
        target_qpos = self.get_ortho6d_from_euler_angle(actions)
    ...
    def parse_hdf5_file_state_only(self, file_path):
        ...
        action_path = os.path.join(file_path, 'actions.npy')
        state_path = os.path.join(file_path, "states.npy")

        actions = np.load(action_path)
        qpos = np.load(state_path)
        qpos = self.get_ortho6d_from_euler_angle(qpos)
