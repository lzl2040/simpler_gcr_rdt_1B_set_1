...
LIBERO_STATE_INDICES = [ STATE_VEC_IDX_MAPPING['eef_pos_x']
...
class RoboticDiffusionTransformerModel(object):
    ...
    def _unformat_action_to_joint(self, action):
        action_indices = LIBERO_STATE_INDICES
    ...
    def to_euler_action(self, raw_actions):
        # print(raw_actions.shape)
        raw_actions = raw_actions.detach().cpu().numpy()
        step = raw_actions.shape[0]
        actions = []
        for i in range(step):
            raw_action = raw_actions[i:i+1] # dim is 10
            action = np.zeros((1, 7))
            action[:,:3] = raw_action[:, :3]
            # print("1")
            rotation_matrix = compute_rotation_matrix_from_ortho6d(raw_action[:, 3:9])
            # print("2")
            euler = convert_rotation_matrix_to_euler(rotation_matrix)
            # print("3")
            action[:, 3:6] = euler
            # print("4")

            action[:, 6] = 2 * (1 - action[:, 6]) - 1 # 
            # print("5")
            # action[:, 6] = 1 if raw_action[:, -1] > 0.5 else -1.0
            # action[:, 6] = -1
    ...
    def step(self, proprio, images, text_embeds):
        ...
        # history of actions
        joints = proprio.to(device).unsqueeze(0)   # (1, 1, 10)
        ...
        actions = self.to_euler_action(trajectory.squeeze(0))
